...
async def apply_text_edits(
    ...
    gate = await maybe_run_tool_preflight(ctx, "apply_text_edits")
    if gate is not None:
...
async def create_script(
    ...
    gate = await maybe_run_tool_preflight(ctx, "create_script")
    if gate is not None:
        return gate.model_dump()
    params: dict[str, Any] = {
        "action": "create",
...
async def delete_script(
    ...
    gate = await maybe_run_tool_preflight(ctx, "delete_script")
    if gate is not None:
        return gate.model_dump()
...
async def manage_script(
    ...
    if action != "read":
        gate = await maybe_run_tool_preflight(ctx, "manage_script", action=action)
        if gate is not None:
            return gate.model_dump()
