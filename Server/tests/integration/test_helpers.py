...
class DummyContext:
    ...
    async def enable_components(self, tags=None, components=None):
        self._visibility_rules.append({
            "enabled": True,
            "tags": sorted(tags or []),
            "components": sorted(components or []),
        })

    async def disable_components(self, tags=None, components=None):
        self._visibility_rules.append({
            "enabled": False,
            "tags": sorted(tags or []),
            "components": sorted(components or []),
    ...
    async def _get_visibility_rules(self):
        return list(self._visibility_rules)
